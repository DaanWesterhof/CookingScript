...
def assignTypes(tokens: [str], last_token: str=None) -> [LEX_Type]:
    ...
    if validateForString(tokens, last_token):
        return [LEX_String(tokens[0])] + assignTypes(tokens[1:], tokens[0])
    ...
    elif tokens[0] in binairyOperators:
        return [LEX_BinairyOperator(tokens[0])] + assignTypes(tokens[1:], tokens[0])
    elif tokens[0] in [':', '"', ',', '.', '->']:
